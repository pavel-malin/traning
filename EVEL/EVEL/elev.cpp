...
void elevator::car_display() // display eleavator image
{
...
{
	int j;
	// flang indicate if destrinations or requests above/below us
	...
	// flags indicate if there is another car, going int the same
	...
	// floor and ditection of ther car (not us)
	int ofloor;  // floor
	...
	// if there's destination on this floor, unload passengers
	if (destination[current_floor] == true)
	{
		destination[current_floor] = false;  // erase distanation
		...
		current_dir != DN))
	{
		...
		// remove floor request for directio we're going
	...
	{							// check floors above
		if (destination[j])   // if destionations
...
void elevator::get_destinations()   // stop, get destinations
{
	...
	cout << "Car " << (car_number + 1)
		<< " has stoppend at floor " << (current_floor + 1)
		<< "\nEneter destination floors (0 when finished)";
